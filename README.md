...
location = pd.DataFrame({
    "site": ["BJFS", "WUHN"], # 站点标识符
    "lat": [39.6, 30.5], # 纬度 (°)
    "lon": [115.9, 114.4], # 经度 (°)
    "alt": [87.4, 25.8],  # WGS84椭球高 (m)
})

zg = ZTDNWMGenerator(
    nwm_path="era5_pl_native_2023010100.nc", # 气象数据文件，包含温度 t (K), 位势 z (m²/s²), 比湿 q (kg/kg)
    location=location,
)
df = zg.run() # 计算站点 ztd
print(df)
...
从数值天气模式数据 (NWM) 计算天顶对流层延迟 (ZTD) 的生成器
...
首先，将位势 $\Phi$ 转换为位势高度 $H_{gp}$ (ECMWF , 2021) :
...
> ECMWF IFS documentation CY47R3 – Part IV: Physical processes, Chapter 12.
>
...
- `vertical_level = "h"`：先将气象参数重采样到固定间隔的椭球高网格，然后在该网格上执行积分。该模式需执行气象参数垂直插值和外推步骤
