...
# GeographicLib/ztdlib/Geoid.cpp from C++ to Python
...
class GeoidBadDataFile(Exception):
    """Raised when a PGM geoid model file is malformed or unreadable."""
...
class GeoidHeight:
    """Geoid undulation calculator from PGM model files.

    Computes the height of the WGS84 geoid above the ellipsoid at any
    given latitude and longitude using cubic or bilinear interpolation.

    Parameters
    ----------
    egm_type : str
        Name of the PGM geoid model file (without extension), e.g.
        ``"egm96-5"`` or ``"egm2008-1"``.  The corresponding
        ``<egm_type>.pgm`` file must exist in the package's
        ``reference/`` directory.

    Raises
    ------
    FileNotFoundError
        If the PGM file cannot be located.
    GeoidBadDataFile
        If the PGM file is malformed.
    """
    ...
    def __init__(self, egm_type: str) -> None:
        self.offset = None
        self.scale = None

        # Search installed package location
        possible_paths = [
            Path(__file__).parent / "reference" / f"{egm_type}.pgm",
        ]
        ...
        if file_path is None:
            ...
            msg = f"Geoid model file '{egm_type}.pgm' not found in:\n  {ref_dir}"
            if "egm2008" in egm_type:
                msg += (
                    f"\n\nEGM2008 files are too large to bundle with this package."
                    f"\nPlease download '{egm_type}.pgm' from GeographicLib and place it in the directory above:"
        ...
        with open(file_path, "rb") as f:
            ...
            # Use ACCESS_READ for Windows compatibility
    ...
    def _rawval0(self, ix: int, iy: int) -> int:
        """Read a raw 16-bit pixel value with boundary wrapping."""
        if iy < 0:
            iy = -iy
            ix += self.width / 2
        elif iy >= self.height:
    ...
    def _rawval(self, ix: int, iy: int) -> int:
        """Read a raw 16-bit pixel value with boundary wrapping."""
        if iy < 0:
            iy = -iy
            ix += self.width // 2  # ensure integer division
        elif iy >= self.height:
            iy = 2 * (self.height - 1) - iy
            ix += self.width // 2  # ensure integer division
        if ix < 0:
    ...
    def get(self, lat: float, lon: float, cubic: bool = True) -> float:
        """Return geoid undulation at a given coordinate.
        ...
        cubic : bool, default True
            Use cubic interpolation.  If False, use bilinear.
