...
class ZTDNWMGenerator:
    ...
    def __init__(
        ...
        time_batch_size : int or None, default None
            If None, all time steps are processed at once (original behavior).
            If set to a positive integer (e.g. 24), the time dimension is split into
            batches of this size, each batch runs the full pipeline independently,
            and results are concatenated. This dramatically reduces peak memory for
            large grids (e.g. ~30x reduction with batch size 24 on 744 time steps).
        """
        # Set progress mode
    ...
    def vertical_interpolate_to_site(self) -> xr.Dataset:
        ...
        # Broadcast h and ztd when vertical_dimension is h
        if self.vertical_dimension == "h":
            h_da, ztd_da = xr.broadcast(ds.h, ds.ztd)
        else:
            h_da = ds.h
            ztd_da = ds.ztd
